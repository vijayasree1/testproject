...
class RadminService
{
    ...
    private function getFirewallTierName ($firewallTierId)
    {
        ...
        $firewallTierDetails = RadminFirewallTier::find($firewallTierId);
    ...
    public function sync( $syncInitiatedBy = 'UNKNOWN', $imsiList = [] )
    {
        ...
        foreach( $radminTerminals as $terminal )
        {
            ...
            if( count($provisioningDetails) > 0 )
            {
                ...
                {
                    array_push($simErrors, "GoDirect Filter setting is not matching. Radmin: {$provisioningDetails['attributes']['firewall_tier_name']}; DBMan: $terminal->Firewall_Filter_Idx"); // $terminal->Firewall_Filter_Idx
                }
